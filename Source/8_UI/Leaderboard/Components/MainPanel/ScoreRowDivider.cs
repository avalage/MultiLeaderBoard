    ...
    internal class ScoreRowDivider : ReeUIComponentV2 {
        ...
        private void ApplyAlpha() {
            if (BeatLeaderDarkGoldTheme.Enabled) {
                var color = BeatLeaderDarkGoldTheme.DividerColor;
                _dividerIcon.color = BeatLeaderDarkGoldTheme.WithAlpha(color, color.a * _currentAlpha);
                return;
            }

            var current = _dividerIcon.color;
            _dividerIcon.color = BeatLeaderDarkGoldTheme.WithAlpha(current, _currentAlpha);
        }
