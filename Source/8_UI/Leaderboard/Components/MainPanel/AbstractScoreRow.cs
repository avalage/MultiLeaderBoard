...
namespace BeatLeader.Components {
    internal abstract class AbstractScoreRow : ReeUIComponentV2 {
        ...
        private static readonly Dictionary<PlayerRole, ColorScheme> DarkGoldColorSchemes = new Dictionary<PlayerRole, ColorScheme> {
            {
                PlayerRole.Default, new ColorScheme(
                    BeatLeaderDarkGoldTheme.BorderGold,
                    BeatLeaderDarkGoldTheme.AccentGold,
                    0.0f,
                    0.0f,
                    0.85f
                )
                ...
                PlayerRole.Tipper, new ColorScheme(
                    BeatLeaderDarkGoldTheme.AccentYellow,
                    BeatLeaderDarkGoldTheme.AccentGold,
                    0.0f,
                    0.0f,
                    0.95f
                ...
                PlayerRole.Supporter, new ColorScheme(
                    BeatLeaderDarkGoldTheme.AccentYellow,
                    BeatLeaderDarkGoldTheme.AccentAmber,
                    0.75f,
                    0.0f,
                    1.0f
                )
        ...
        private static ColorScheme GetColorScheme(PlayerRole playerRole) {
            var schemes = BeatLeaderDarkGoldTheme.Enabled ? DarkGoldColorSchemes : ColorSchemes;
            return schemes.TryGetValue(playerRole, out var scheme) ? scheme : schemes[PlayerRole.Default];
        }
        ...
        protected override void OnInitialize() {
            ...
            if (BeatLeaderDarkGoldTheme.Enabled) {
                ...
                _background.sprite = BeatLeaderDarkGoldTheme.SolidPanelSprite;
            } else {
        ...
        protected void UpdateContent() {
            ...
            switch (ScoreRowContent) {
                case Score playerScore: {
                    ...
                    colorScheme = GetColorScheme(supporterRole);
                    break;
                }
                ...
                case ClanScore clanScore: {
                    ...
                    colorScheme = GetColorScheme(PlayerRole.Default);
                    break;
                }
                default: {
                    foreach (var cell in _cells.Values) {
            ...
            foreach (var pair in _cells) {
                if (ScoreRowContent.ContainsValue(pair.Key)) pair.Value.SetValue(ScoreRowContent.GetValue(pair.Key));
                    ...
                    pair.Value.SetValue(null);
                    pair.Value.MarkEmpty();
                }
            }
        }
        ...
        private void UpdateVisualsIfDirty() {
            ...
            var idleHighlight = BeatLeaderDarkGoldTheme.Enabled && highlight ? 1.0f : colorScheme.IdleHighlight;
            var hoverHighlight = BeatLeaderDarkGoldTheme.Enabled && highlight ? 1.0f : colorScheme.HoverHighlight;
            if (BeatLeaderDarkGoldTheme.Enabled && highlight) {
                idleHighlight = 0.0f;
                hoverHighlight = 0.22f;
            }

            _underline.DefaultColor = new Color(idleHighlight, 0, 0, _currentAlpha);
            _underline.HighlightColor = new Color(hoverHighlight, 0, 0, _currentAlpha);
            if (BeatLeaderDarkGoldTheme.Enabled) {
                _background.material = GameResources.UINoGlowMaterial;
                _background.sprite = BeatLeaderDarkGoldTheme.SolidPanelSprite;
                _background.type = Image.Type.Simple;
                if (highlight) {
                    var color = BeatLeaderDarkGoldTheme.CurrentPlayerScoreBackgroundColor;
        ...
        protected ColorScheme colorScheme = GetColorScheme(PlayerRole.Default);
