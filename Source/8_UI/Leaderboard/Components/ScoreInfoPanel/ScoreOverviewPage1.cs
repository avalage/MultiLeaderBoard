...
namespace BeatLeader.Components {
    internal class ScoreOverviewPage1 : ReeUIComponentV2 {
        ...
        public void SetScore(Score score) {
            ...
            var fcPerformancePoints = new ScoreRowPerformancePoints(
                score.fcPp,
                score.HasVisibleBeatLeaderPp && score.fcPp != 0,
                score.scoreSaberFcPp,
                score.withScoreSaberFcPp
            );
            var hasFcPerformancePoints = fcPerformancePoints.HasBeatLeaderValue || fcPerformancePoints.HasScoreSaberValue;
            _ppText.Text1 = GetStringWithLabel(
                FormatUtils.FormatPerformancePoints(score.PerformancePoints),
                "pp " + (!score.fullCombo && hasFcPerformancePoints ? $"<size=60%><color={Good}>[?]</color></size>" : ""));
            _ppText.Text2 = GetStringWithLabel(
                FormatUtils.FormatPerformancePoints(fcPerformancePoints),
                $"<color={Good}>FC pp</color>");
            _ppText.HoverEnabled = !score.fullCombo && hasFcPerformancePoints;
        ...
        private static string Neutral => BeatLeaderDarkGoldTheme.TextPrimaryHtml;
        private static string Faded => BeatLeaderDarkGoldTheme.TextSecondaryHtml;
        private static string Good => BeatLeaderDarkGoldTheme.SuccessGreenHtml;
        private static string Bad => BeatLeaderDarkGoldTheme.WarningOrangeHtml;
