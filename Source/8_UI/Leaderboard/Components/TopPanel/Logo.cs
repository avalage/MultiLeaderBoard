...
namespace BeatLeader.Components {
    internal class Logo : ReeUIComponentV2 {
        ...
        private const string DarkGoldLogoResource = Plugin.ResourcesPath + ".Images.dark_gold_logo.png";
        private static Sprite? _darkGoldLogoSprite;
        private Material? _materialInstance;
        private bool _usingStaticLogo;

        private void SetMaterial() {
            if (BeatLeaderDarkGoldTheme.Enabled && TryUseDarkGoldLogo()) {
                return;
            }
        ...
        private bool TryUseDarkGoldLogo() {
            try {
                _darkGoldLogoSprite ??= LoadDarkGoldLogoSprite();
                _logoImage.material = GameResources.UINoGlowMaterial;
                _logoImage.sprite = _darkGoldLogoSprite;
                _logoImage.type = Image.Type.Simple;
                _logoImage.color = BeatLeaderDarkGoldTheme.LogoTint;
                _logoImage.preserveAspect = true;
                _logoImage.rectTransform.localScale = new Vector3(1.08f, 1.08f, 1.0f);
                var clickableImage = _logoImage.GetComponent<ClickableImage>();
                if (clickableImage != null) {
                    clickableImage.DefaultColor = BeatLeaderDarkGoldTheme.LogoTint;
                    clickableImage.HighlightColor = BeatLeaderDarkGoldTheme.LogoTint;
                }
                _usingStaticLogo = true;
                return true;
            } catch (System.Exception ex) {
                Plugin.Log.Warn($"[DarkGoldTheme] Failed to load dark gold logo: {ex.Message}");
                return false;
            }
        }

        private static Sprite LoadDarkGoldLogoSprite() {
            using var stream = ResourcesUtils.GetEmbeddedResourceStream(DarkGoldLogoResource);
            if (stream == null) {
                throw new System.InvalidOperationException($"Embedded resource not found: {DarkGoldLogoResource}");
            }
            ...
            if (!texture.LoadImage(memoryStream.ToArray())) {
                throw new System.InvalidOperationException("Unity failed to decode dark gold logo PNG.");
            }

            texture.name = "BeatLeaderDarkGoldLogoTexture";
            ...
            sprite.name = "BeatLeaderDarkGoldLogo";
            return sprite;
        }
