    ...
    public abstract class PersistentSingletonWebRequestBase<T, TResult, TDescriptor> : PersistentWebRequestBase
        ...
        public static event WebRequestStateChangedDelegate<IWebRequest<TResult>>? StateChangedEvent {
            add {
                ...
                InvokeStateChangedHandler(value, Instance, Instance != null ? RequestState : RequestState.Uninitialized, Instance != null ? FailReason : null);
            }
            remove {
                StateChangedEventInternal -= value;
            }
        ...
        internal static void Instance_StateChangedEvent(IWebRequest<TResult> instance, RequestState state, string? failReason) {
            UnityMainThreadTaskScheduler.Factory.StartNew(() => {
                    InvokeStateChangedHandlers(instance, state, failReason);
                }
            ).RunCatching();
        }

        private static void InvokeStateChangedHandlers(IWebRequest<TResult> instance, RequestState state, string? failReason) {
            var handlers = StateChangedEventInternal;
            if (handlers == null) return;

            foreach (WebRequestStateChangedDelegate<IWebRequest<TResult>> handler in handlers.GetInvocationList()) {
                InvokeStateChangedHandler(handler, instance, state, failReason);
            }
        }

        private static void InvokeStateChangedHandler(WebRequestStateChangedDelegate<IWebRequest<TResult>>? handler, IWebRequest<TResult>? instance, RequestState state, string? failReason) {
            if (handler == null) return;

            try {
                handler(instance!, state, failReason);
            } catch (Exception ex) {
                Plugin.Log.Error($"[{typeof(T).Name}] State handler exception: {ex}");
            }
        }
        ...
        internal static void Instance_ProgressChangedEvent(IWebRequest<TResult> instance, float downloadProgress, float uploadProgress, float overallProgress) {
            UnityMainThreadTaskScheduler.Factory.StartNew(() => {
                    InvokeProgressChangedHandlers(instance, downloadProgress, uploadProgress, overallProgress);
                }
            ).RunCatching();
        }

        private static void InvokeProgressChangedHandlers(IWebRequest<TResult> instance, float downloadProgress, float uploadProgress, float overallProgress) {
            var handlers = ProgressChangedEventInternal;
            if (handlers == null) return;

            foreach (WebRequestProgressChangedDelegate<IWebRequest<TResult>> handler in handlers.GetInvocationList()) {
                try {
                    handler(instance, downloadProgress, uploadProgress, overallProgress);
                } catch (Exception ex) {
                    Plugin.Log.Error($"[{typeof(T).Name}] Progress handler exception: {ex}");
                }
            }
        }
    }
