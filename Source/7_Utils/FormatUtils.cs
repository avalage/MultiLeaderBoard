...
namespace BeatLeader {
    internal static class FormatUtils {
        ...
        public static string FormatPerformancePoints(ScoreRowPerformancePoints value) {
            if (value.HasBeatLeaderValue && value.HasScoreSaberValue) {
                return $"<color={BeatLeaderPPColor}>{value.BeatLeaderValue:F1}</color><color={BeatLeaderDarkGoldTheme.TextMutedHtml}>/</color><color={ScoreSaberPPColor}>{value.ScoreSaberValue:F1}<size=70%>pp</size></color>";
            }

            if (value.HasBeatLeaderValue) {
                return FormatBeatLeaderPP(value.BeatLeaderValue);
            }

            return value.HasScoreSaberValue ? FormatScoreSaberPP(value.ScoreSaberValue) : string.Empty;
        }
