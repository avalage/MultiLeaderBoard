...
namespace BeatLeader {
    internal static class FormatUtils {
        ...
        private static readonly Color LegacyOldScoreColor = new(0.5f, 0.5f, 0.5f);
        private static readonly Color LegacyFreshScoreColor = new(1f, 1f, 1f);
        ...
        private static string GetTimesetColorString(TimeSpan timeSpan) {
            ...
            var oldScoreColor = BeatLeaderDarkGoldTheme.Enabled ? BeatLeaderDarkGoldTheme.TextMuted : LegacyOldScoreColor;
            var freshScoreColor = BeatLeaderDarkGoldTheme.Enabled ? BeatLeaderDarkGoldTheme.TextSecondary : LegacyFreshScoreColor;
        ...
        public static string MarkPhrase(string text, string phrase) {
            return MarkPhrase(text, phrase, BeatLeaderDarkGoldTheme.Enabled ? BeatLeaderDarkGoldTheme.AccentYellow : new(0.8f, 0f, 1f, 1f), true);
        }
        ...
        public static string FormatRank(int rank, bool withPrefix) {
            var value = rank is -1 ? "?" : rank.ToString();
            return BeatLeaderDarkGoldTheme.Enabled
                ? $"<color={BeatLeaderDarkGoldTheme.AccentYellowHtml}>{(withPrefix ? "<size=70%>#</size>" : "")}{value}</color>"
                : $"{(withPrefix ? "<size=70%>#</size>" : "")}{value}";
        }
        ...
        private static string ModifiersColor => BeatLeaderDarkGoldTheme.TextSecondaryHtml;
        ...
        private static string GetAccColorString(float acc) {
            if (BeatLeaderDarkGoldTheme.Enabled) {
                const float goldStartAcc = 0.7f;
                if (acc <= goldStartAcc) {
                    return ColorUtility.ToHtmlStringRGBA(BeatLeaderDarkGoldTheme.AccuracyNeutralColor);
                }

                var goldLerpValue = Mathf.InverseLerp(goldStartAcc, 1.0f, Mathf.Clamp01(acc));
                goldLerpValue = goldLerpValue * goldLerpValue * (3.0f - 2.0f * goldLerpValue);
                var themedColor = goldLerpValue < 0.58f
                    ? Color.Lerp(BeatLeaderDarkGoldTheme.AccuracyNeutralColor, BeatLeaderDarkGoldTheme.AccentGold, goldLerpValue / 0.58f)
                    : Color.Lerp(BeatLeaderDarkGoldTheme.AccentGold, BeatLeaderDarkGoldTheme.AccentYellow, (goldLerpValue - 0.58f) / 0.42f);
        ...
        private static string BeatLeaderPPColor => BeatLeaderDarkGoldTheme.BeatLeaderPpHtml;
        private static string ScoreSaberPPColor => BeatLeaderDarkGoldTheme.ScoreSaberPpHtml;
        private static string AccSaberAPColor => BeatLeaderDarkGoldTheme.AccSaberApHtml;

        public static string FormatPP(float value) {
            return FormatBeatLeaderPP(value);
        }

        public static string FormatBeatLeaderPP(float value) {
            return FormatPPWithColor(value, BeatLeaderPPColor);
        }

        public static string FormatScoreSaberPP(float value) {
            return FormatPPWithColor(value, ScoreSaberPPColor);
        }

        public static string FormatAccSaberAP(float value) {
            return $"<color={AccSaberAPColor}>" + (value < 0 ? "?" : $"{value:F2}<size=70%>ap</size>") + "</color>";
        }
        ...
        public static string FormatPerformancePoints(ScoreRowPerformancePoints value) {
            ...
            if (value.HasScoreSaberValue) {
                parts.Add($"<color={ScoreSaberPPColor}>{value.ScoreSaberValue:F1}<size=70%>pp</size></color>");
            }

            if (value.HasAccSaberValue) {
                parts.Add($"<color={AccSaberAPColor}>{value.AccSaberValue:F1}<size=70%>ap</size></color>");
            }
        ...
        private static string FormatPPWithColor(float value, string color) {
            return $"<color={color}>" + (value is -1 ? "?" : $"{value:F2}<size=70%>pp</size>") + "</color>";
        }
        ...
        public static string FormatStars(float value) {
            return BeatLeaderDarkGoldTheme.Enabled
                ? $"<color={BeatLeaderDarkGoldTheme.AccentGoldHtml}>{value:f2}<size=70%>★</size>"
                : $"{value:f2}<size=70%>★</size>";
        }
        ...
        public static string FormatPauses(int value) {
            return $"{value}<size=50%> <b><color={BeatLeaderDarkGoldTheme.TextSecondaryHtml}>ll";
        }
